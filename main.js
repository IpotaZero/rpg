...
const data = {
  ...
  items: [],
  task: "保健室のプリンに話しかける",
  flag: { meet_purine: true, key_north: false }
}
...
const scene_menu = new class extends Scene {
  ...
  draw() {
    ...
    Ifont({ size: 60, colour: "black" })
    Itext(null, 120, 390, "Aqua")
  }
}()
...
const scene_battle = new class extends Scene {
  constructor() {
    ...
    this.command = new Icommand(30, 60, 24, { "": ["たたかう", "にげる"], "0": ["アクア", "プリン", "アモン", "シトリ"], "0.": ["こうげき", "アルゴリズム", "アイテム"] },
      ...
      {
        "0": (me) => {
          Irect(150, 40, 120, 24, "black")
          Irect(150, 40, 120 * (this.aqua.meter / 100), 24, "#c0c0c0")

          Irect(300, 40, 120, 24, "black")
          Irect(300, 40, 120 * (this.aqua.hp / this.aqua.max_hp), 24, "#ff4040")

          Irect(450, 40, 120, 24, "black")
          Irect(450, 40, 120 * (this.aqua.token / this.aqua.max_token), 24, "#4040ff")
        },
        "0.0": (me) => {
          this.enemy.hp -= this.aqua.attack
          this.aqua.meter = 0

          this.log.push("アクアはウィルスにこうげきした！")
  ...
  loop() {
    ...
    if (this.enemy.hp <= 0 || this.enemy.token <= 0) {
      scene_manager.move_to(scene_main)
    }
  ...
  draw() {
    ...
    Irect(20, 20, 580, 160, "#c0c0c0", "stroke", 12)
    Irect(600, 20, 460, 160, "#c0c0c0", "stroke", 12)

    Ifont({ size: 24, colour: "#c0c0c0", font: "serif" })
    Itext4(null, 640, 60, font_size, this.log)
...
const back_paper = new Iimage("images/Paper01.jpg", 0, 0, width, height, { alpha: 0.2, camera: false })
